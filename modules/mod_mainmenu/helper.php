...
class modMainMenuHelper
{
	...
	function &getXML($type, &$params, $decorator)
	{
		...
		if(!$active && JRequest::getVar('view') == 'type'){
			$menu->setActive(7);
			$active	= $menu->getItem(7);
		}
...
class JMenuTree extends JTree
{
	...
	function toXML()
	{
		...
		{
			$this->_buffer .= '<ul class="sub-menu">';
	...
	function _getItemData(&$params, $item)
	{
		...
		if ($item->type == 'menulink')
		{
			...
			if ($newItem = $menu->getItem($item->query['Itemid'])) {
    			$tmp = clone($newItem);
				$tmp->name	 = '<span class="title"><![CDATA['.$item->name.']]></span>';
				$tmp->mid	 = $item->id;
		...
		if($iParams->get('menu_icon') && $iParams->get('menu_icon') != -1){
			$icon = '<i class="fa '.$iParams->get('menu_icon').'"> </i>  ';
		}else{
			$icon = null;
		}
		switch ($tmp->type)
		{
			case 'separator' :
				return '<a href="javascript:void(0)">'.$icon.' '.$image.$tmp->name.'  <span class="icon-arrow"></span></a>';
				break;
		...
		if ($tmp->url != null)
		{
			...
			switch ($tmp->browserNav)
			{
				...
				case 0:
					// _top
					$data = '<a href="'.$tmp->url.'">'.$icon.$image.$tmp->name.'</a>';
					break;
				case 1:
					// _blank
					$data = '<a href="'.$tmp->url.'" target="_blank">'.$icon.$image.$tmp->name.'</a>';
					break;
				case 2:
					// window.open
