...
class CCKjSeblodItem_Form
{
	...
	function getDisplayVariation( $data, $formdisplay, $value, $text, $form, $itemId, $itemName, $html, $more = '' ) {
		$data->form		=	'<input class="form-control form-control" type="hidden" id="'.$itemId.'" name="'.$itemName.'" value="'.$value.'" />';		
	...
	function getDataII( &$item, $itemValue, $itemId, $itemName, $client, $artId, $fullscreen, $actionMode, &$row, $lang_id, $cat_id, $k = -1, $ran ) {
		...
		switch ( $item->typename ) {
				case 'external_article':
					...
					$form		= 	JHTML::_( 'select.genericlist', $articles, $itemName, 'class="form-control select '.$required.'" size="1"', 'value', 'text', $value, $itemId );
					if ( ! $item->formdisplay ) {
						$data->form	=	$form;
					} else {
						$text	=	'';
				...
				case 'joomla_readmore':
					...
					if ( ( $client == 'admin' && ( $item->displayfield == 0 || $item->displayfield == 1 ) )
					  || ( $client == 'site' && ( $item->displayfield == 0 || $item->displayfield == 2 ) )  ) { 
						$form	=	JHTML::_( 'select.booleanlist', $itemName, 'class="form-control radio"', $value );
					} else {
						$form	= 	'<input class="form-control" type="hidden" id="'.$itemId.'" name="'.$itemName.'" value="'.$value.'" />';
					}
					if ( ! $item->formdisplay ) {
						$data->form	=	$form;
					} else {
				...
				case 'joomla_module':
					...
					if ( ! $item->formdisplay ) {
						$modules = JModuleHelper::getModules($item->defaultvalue);
						$form = '';
						for($mod=0; $mod < count($modules); $mod++){
							$form	.= JModuleHelper::renderModule($modules[$mod]);
						} 
						$form	.=	'<input class="'.$item->css.'" type="hidden" value="'.$value.'" name="'.$itemName.'" />';
						$data->form	=	$form;
					} else {
				...
				case 'joomla_plugin_button':
					...
					if ( $value ) {
						$form	.=	'<input class="form-control" type="hidden" id="'.$itemNameH.'_hidden" name="'.$itemNameH.'_hidden" value="'.htmlspecialchars($value).'" />';
					}
					if ( ! $item->formdisplay ) {
						$data->form	=	$form;
					} else {
				...
				case 'form_action':
					...
					$form	=	( $client == 'admin' ) ? '' : '<form target="'.$target.'" enctype="multipart/form-data" action="'.JURI::base().'index.php?option=com_cckjseblod&amp;view=type&amp;layout=form&amp;task=save&amp;cckid='.$artId.'&amp;Itemid='._ERROR_REFRESH_ITEMID.'" method="post" id="'.$itemName.$ran.'" name="'.$itemName.$ran.'">';
					$data->form		=	$form;
					$data->value	=	'';
					break;
				case 'captcha_image':
					if ( $artId && ( $item->gEACL == -1 || ( $item->gEACL == 1 && $client == 'site' ) ) ) {
						...
						$form		.=	'<input class="form-control text '.$required.'" type="text"  name="cptcsecure" value="" size="'.$item->size.'" />';
					}
					if ( ! $item->formdisplay ) {
						$data->form	=	$form;
					} else {
				...
				case 'email':
					...
					if ( $item->displayfield == 0 ) {
						...
						$form		=	'<input class="form-control text '.$required.$validation.'" type="text" id="'.$itemId.'" name="'.$itemName.'" maxlength="250" size="'.$item->size.'" value="'.$value.'" />';
					} else {
						$form		= 	'<input class="form-control" type="hidden" id="'.$itemId.'" name="'.$itemName.'" value="'.$value.'" />';
					}
					if ( ! $item->formdisplay ) {
				...
				case 'save':
					...
					if ( $item->bool2 == 2 ) {
						...
						$section = (int)$item->bool3;
						$options	=	CCKjSeblodItem_Form::getObjectListFromDatabase( 'SELECT title AS text, id AS value FROM #__categories WHERE section = '.$section.' ORDER BY title' );
					}else {
					...
					if ( $item->format != 'generic' ) {
						...
						$form			=	HelperjSeblod_Helper::radioList( $opts, $itemName, 'class="form-control radio '.$required.'" size="1"', 'value', 'text', $value, $itemId, $orientation );
						$html			=	'<input';
					} else {
						$required	=	( $item->required ) ? 'required required-enabled' : '';
						$form		= 	JHTML::_( 'select.genericlist', $opts, $itemName, 'class="form-control select '.$required.'" size="1"', 'value', 'text', $value, $itemId, $orientation);			
						$html		=	'<select';
					}
					if ( ! $item->formdisplay ) {
						$data->form	=	$form;
				...
				case 'button_submit':
				  if ( $ran ) {
					   $form	=	'<button class="'.$item->css.' btn btn-default" type="submit" onclick="javascript: submitbutton'.$ran.'(\'save\');" name="'.$itemName
					   			.	'" style="'.$item->style.'">'.$item->label.'</button>';
					} else {
					   $form	=	'<button class="'.$item->css.' btn btn-default" type="submit" onclick="javascript: submitbutton(\'save\');" name="'.$itemName
					   			.	'" style="'.$item->style.'">'.$item->label.'</button>';
          			}
					if ( ! $item->formdisplay ) {
						$data->form	=	$form;
				...
				case 'checkbox':
					...
					$form		=	HelperjSeblod_Helper::checkBoxList( $opts, $itemName.'[]', 'class="grey '.$required.'"', 'value', 'text', $value, $itemId, $orientation, $item->cols, $item->bool2 );
					if ( ! $item->formdisplay ) {
						$data->form	=	$form;
					} else {
				...
				case 'password':
					...
					$required	=	( $item->required ) ? 'required required-enabled' : '';
					$validation	=	( $item->validation ) ? ' ' . $item->validation : '';
					$form	=	'<input class="form-control password minLength '.$required.$validation.'" validatorProps="{minLength:'.$minlength.'}" type="password" id="'.$itemId.'" name="'.$itemName.'" maxlength="'.$item->maxlength.'" size="'.$item->size.'" value="'.$value.'" />';
					if ( ! $item->formdisplay ) {
						$data->form	=	$form;
					} else {
						$data	=	CCKjSeblodItem_Form::getDisplayVariation( $data, $item->formdisplay, $value, $value, $form, $itemId, $itemName, '<input' );
				...
				case 'radio':
					...
					$orientation	=	( $item->bool ) ? true : false;
					$form			=	HelperjSeblod_Helper::radioList( $opts, $itemName, 'class="form-control radio '.$required.'" size="1"', 'value', 'text', $value, $itemId, $orientation, $item->cols, $item->bool2 );
					if ( ! $item->formdisplay ) {
						$data->form	=	$form;
					} else {
				...
				case 'text':
					...
					//$form		=	'<input class="form-control text '.$required.$validation.'" type="text" id="'.$itemId.'" name="'.$itemName.'"'
								//.	' maxlength="'.$item->maxlength.'" size="'.$item->size.'" value="'.$value.'" '.$style.' />';
					$form 		= '<input placeholder="'.$item->defaultvalue.'" class="form-control" id="'.$itemId.'" name="'.$itemName.'" type="text" value="'.$value.'" maxlength="'.$item->maxlength.'" size="'.$item->size.'" '.$style.' '.$required.'>';
					if ( $item->bool8 ) {
				...
				case 'select_dynamic':
					...
					$form		=	JHTML::_( 'select.genericlist', $opts, $itemName, 'class="form-control select form-control '.$required.'" size="1" '.$disabled, 'value', 'text', $value, $itemId );
					// If Ajax Parent !
					...
					if ( ( $item->bool == 2 || $item->bool == 3 ) && $item->extended && $item->elemxtd == 'item' && $item->bool2 != 1 ) {
						...
						$control = ( $client == 'site' ) ? '' : '&controller=interface';
						$script = '$("#'.$itemId.'").on("change",function(){
								var val = $(this).val();
								...
								var fieldname = "#'.str_replace('jcontent','',$itemInfo->name).'";
								var field = $(fieldname);
								$.ajax({
									url: url,
									method:"get"
								}).done(function(data){
									field.empty().append($(data).html())
								})
							});
							';
						$scriptjs = '<script language="javascript" type="text/javascript">$(document).ready(function(e) {'.$script.'});</script>';
						echo $scriptjs;
					}
					if ( ! $item->formdisplay ) {
						$data->form	=	$form;
				...
				case 'select_multiple':
					...
					$form	=	JHTML::_( 'select.genericlist', $opts, $itemName.'[]', 'class="form-control select form-control '.$required.'" multiple="multiple" size="'.$size.'"', 'value', 'text', $value, $itemId );
					if ( ! $item->formdisplay ) {
						$data->form	=	$form;
					} else {
						$text	=	CCKjSeblodItem_Form::getOptionText( $itemValue, $item->options, 1, $item->divider );
				...
				case 'select_numeric':
					...
					$form	=	JHTML::_( 'select.genericlist', $opts, $itemName, 'class="form-control select form-control '.$required.'" size="1"', 'value', 'text', $value, $itemId );
					if ( ! $item->formdisplay ) {
						$data->form	=	$form;
					} else {
						$data	=	CCKjSeblodItem_Form::getDisplayVariation( $data, $item->formdisplay, $value, $value, $form, $itemId, $itemName, '<select' );
				...
				case 'select_simple':
					...
					$form	=	( sizeof( $opts ) ) ? JHTML::_( 'select.genericlist', $opts, $itemName, 'class="form-control select form-control'.$required.'" size="1"', 'value', 'text', $value, $itemId ) : '';
					if ( ! $item->formdisplay ) {
						$data->form	=	$form;
					} else {
						$text	=	CCKjSeblodItem_Form::getOptionText( $value, $item->options );
				...
				case 'textarea':
					...
					$form		= 	'<textarea class="textarea form-control '.$required.$validation.'" id="'.$itemId.'" name="'.$itemName.'" cols="'.$item->cols.'" rows="'.$item->rows.'" '.$maxlength. 'style="'.$item->style.'" >'.$value.'</textarea>';
					if ( ! $item->formdisplay ) {
						$data->form	=	$form;
					} else {
				...
				case 'page_builder':
					...
					$validation	= 	( $item->validation ) ? ' ' . $item->validation : '';
					$maxlength	=	( $item->maxlength ) ? 'onkeydown="this.value=this.value.substring(0, '.$item->maxlength.');"' : '';
					//echo'<pre>';print_r(dirname(dirname(__FILE__)));
					
						include_once ( dirname(dirname(__FILE__)) .DS.'builder'.DS.'builder_layout.php' );
						$form		= 	'<div id="basic">';
						$itemValue = html_entity_decode($itemValue);
						$form		.= builder_layout(json_decode($itemValue));
						$form		.= 	'<textarea class="textarea '.$required.$validation.'" style="display: none;" id="jform_sptext" name="'.$itemName.'" cols="'.$item->cols.'" rows="'.$item->rows.'" '.$maxlength. 'style="'.$item->style.'" >'.$value.'</textarea>';
						$form		.= '</div>';
						
					
						$data	=	CCKjSeblodItem_Form::getDisplayVariation( $data, $item->formdisplay, $value, $value, $form, $itemId, $itemName, '<input' );
						include_once ( dirname(dirname(__FILE__)) .DS.'builder'.DS.'parser'.DS.'addon-parser.php' );
						$itemValue = html_entity_decode($itemValue);
						$value  = 	AddonParser::viewAddons(json_decode($itemValue));
					...
					if ( ! $user->id ) {
						...
						if ( $item->bool ) {	// Default
							...
							if ( $mainframe->isSite() && strpos( $item->format, 'tinypreset_' ) === false ) {
								...
								$form	=	$wysEditor->display( $itemName, $value, '100%', $item->height, '60', '20', array('pagebreak', 'readmore', 'cckjseblod') );
							...
							$extra	=	'<textarea class="form-control" style="display: none;" id="'.$itemId.'-wysiwyg" name="'.$itemName.'" cols="25" rows="3" style="overflow:hidden;">'
										.$value.'</textarea>';
							if ( strpos( $itemName, '[]' ) !== false ) {
								$itemNameH	=	substr( $itemName, 0, -2 );
								$extra2 	=	'<input class="form-control" type="hidden" id="'.$itemId.'-wysiwyg_hidden" name="'.$itemNameH.'_hidden[]" value="" />';
							} else if ( $itemName[(strlen($itemName) - 1 )] == ']' ) {
								$itemNameH	=	substr( $itemName, 0, -1 );
								$extra2 	=	'<input class="form-control" type="hidden" id="'.$itemId.'-wysiwyg_hidden" name="'.$itemNameH.'_hidden]" value="" />';
							} else {
								$extra2 	= 	'<input class="form-control" type="hidden" id="'.$itemId.'-wysiwyg_hidden" name="'.$itemName.'_hidden" value="" />';
							}
							$modal	= 	HelperjSeblod_Display::quickModalWysiwygJs( 'Editor', '', $itemId.'-wysiwyg', 'pagebreak', $required, -1, $item->format, 1000, 560 );
							$form	=	$extra.$extra2.$modal;
							
							$itemNameH	=	$itemId.'-wysiwyg_hidden';
							$script		=	'$("'.$itemNameH.'").value = $("'.$itemId.'-wysiwyg").value';
							echo '<script language="javascript" type="text/javascript">$(document).ready(function(){'.$script.'});</script>';
				...
				case 'file':
					...
					if ( $item->bool4 ) {
						...
						$list = JHTML::_( 'select.genericlist', $optionsFileList, $itemName.'[]', 'class="form-control select '.$required.'" size="'.$item->rows.'" multiple="multiple"', 'value', 'text', $selectedFileList );
					} else {
						$list = JHTML::_( 'select.genericlist', $optionsFileList, $itemName, 'class="form-control select '.$required.'" size="1"', 'value', 'text', $selectedFileList );
					}
					if ( $item->bool2 ) {
						$myoutput = '<input class="form-control text notrequired-disabled" type="text" id="'.$itemName.'_location" name='.$itemName.'_location" maxlength="250"'
								  . 'size="32" value="'.$item->location.'" disabled="disabled" />';
					}
					$itemNameH = (strpos($itemName, '[]') !== false) ? substr( $itemName, 0, -2 ) : $itemName;
					@$myoutput .= '<input class="form-control" type="hidden" id="'.$itemNameH.'_hidden" name="'.$itemNameH.'_hidden" value="'.$item->location.'" />';
					$form	=	$myoutput.$list;
					$value	=	$itemValue;
				...
				case 'folder':
					...
					$list = JHTML::_( 'select.genericlist', $optionsFileList, $itemName, 'class="form-control select '.$required.'" size="1"', 'value', 'text', $selectedFolder );
					if ( $item->bool2 ) {
						$myoutput = '<input class="form-control text notrequired-disabled" type="text" id="'.$itemName.'_location" name='.$itemName.'_location" maxlength="250" size="32" value="
				...
				case 'upload_image':
					...
					if ( $item->bool2 ) {
						$extra		= 	'<input class="form-control text '.$required.'" type="text" id="'.$itemName.'_location" name='.$itemName.'_location" maxlength="250" size="32" value="'
					...
					if ( strpos( $itemName, '[]' ) !== false ) { //FieldX
						$itemNameH	=	substr( $itemName, 0, -2 );
						$form_more 	=	'<input type="hidden" id="'.$itemNameH.'_hidden[]" name="'.$itemNameH.'_hidden[]" value="'.$value.'" />';						
					} else if ( $itemName[(strlen($itemName) - 1 )] == ']' ) { //GroupX
						$itemNameH	=	substr( $itemName, 0, -1 );
						$form_more 	=	'<input type="hidden" id="'.$itemNameH.'_hidden]" name="'.$itemNameH.'_hidden]" value="'.$value.'" />';
						if ( $item->bool7 && $itemValue ) {
							$onchange	=	'onchange="$(\'#'.$itemName.'_delete\').prop( \'checked\' , true);"';
							$chkbox		=	'<input type="checkbox" id="'.$itemName.'_delete" name="'.$itemNameH.'_delete]" value="1" />';
						}
					} else { //Default
						$form_more	=	'<input type="hidden" id="'.$itemName.'_hidden" name="'.$itemName.'_hidden" value="'.$value.'" />';
						if ( $item->bool7 && $itemValue ) {
							$onchange	=	'onchange="$(\'#'.$itemName.'_delete\').prop( \'checked\' , true);"';
							$chkbox		=	'<input type="checkbox" id="'.$itemName.'_delete" name="'.$itemName.'_delete" value="1" />';
						}
					}
					$form	=	'<input class="file" accept="image/*" type="file" id="'.$itemName.'" name="'.$itemName.'" size="'.$item->size.'" '.$onchange.'  />';
					$form	=	@$extra.$form.$form_more;
					if ( $chkbox != '' ) {
						$span	=	'<span class="DescriptionTip hidden" title="'.JText::_( 'DELETE FILE' ).'::'.JText::_( 'DELETE FILE DESCRIPTION' ).'">';
						$form	.=	'&nbsp;&nbsp;'.$span.$chkbox.'</span>';
					}
					//$link		=	'javascript: SqueezeBox.fromElement(\''.JURI::root().$itemValue.'\', {handler: \'image\'});';
					$datalink		=	JURI::root().$itemValue;
					//$editlink	=	'<a class="imgedit" onclick=\'window.open("'.JURI::root().'/plugins/system/phpimageeditor/index.php?isadmin=false&imagesrc='.urlencode(getcwd().'/'.$itemValue).'&language=en-GB", "editor", "width=1000, height=600");\'>Edit</a>';
					if ( $item->bool6 != -1 ) {
						$title	=	strrpos( $itemValue, '/' ) ? substr( $itemValue, strrpos( $itemValue, '/' ) + 1 ) : $itemValue;
						if ( $item->bool6 > 1 ) {
							// 2 // 3 // 4 // 5
							if ( $item->bool6 == 2 ) {
								...
								$preview	=	'<img class="img-thumbnail" src="'.JURI::root().$itemValue.'" alt="" '.$width.' '.$height.' style="padding-bottom: 1px;" />'.$editlink;
								$form		=	'&nbsp;&nbsp;'.$preview.''.$form;								
							} else {
								...
								if(!$itemValue){
									$thumb = '<img src="'.JURI::root().'images/aa.png" alt=""/>';
								}else{
									$thumb = '<img src="'.JURI::root().$location.$suffix.$suffix2.'_thumb'.($item->bool6 - 2).'/'.$title.'" />';
								}
								$preview	=	'<div class="fileupload-new thumbnail">'.$thumb.'</div><div class="fileupload-preview fileupload-exists thumbnail"></div>';
								$form = '<div class="btns"><span class="btn btn-light-grey btn-file"><span class="fileupload-new"><i class="fa fa-picture-o"></i> Select image</span><span class="fileupload-exists"><i class="fa fa-picture-o"></i> Change</span>'.$form.'</span><a onmousedown="$(\'#pr_img_delete\').prop( \'checked\' , false);" href="#" class="btn fileupload-exists btn-light-grey" data-dismiss="fileupload" ><i class="fa fa-times"></i> Cancel</a></div>';
								$form		=	'<div class="fileupload fileupload-new" data-provides="fileupload">'.$preview.''.$form.'</div>';	
							}
						} else if ( $item->bool6 == 1 ) {
							$preview	=	'<a href="javascript:void(0);" data-link="'.$datalink.'" onclick="'.$link.'"><img src="'.JURI::root().'/media/jseblod/_icons/preview-default.png" alt="" /></a>';
							$form		.=	'&nbsp;&nbsp;'.$preview;
						} else {
							$preview	=	'<a href="javascript:void(0);" data-link="'.$datalink.'" onclick="'.$link.'">'.$title.'</a>';
							$form		.=	'&nbsp;&nbsp;'.$preview;
						}
					}
					if ( ! $item->formdisplay ) {
				...
				case 'upload_simple':
					...
					if ( strpos( $itemName, '[]' ) !== false ) { //FieldX
						$itemNameH	=	substr( $itemName, 0, -2 );
						$form_more 	=	'<input class="form-control" type="hidden" id="'.$itemNameH.'_hidden[]" name="'.$itemNameH.'_hidden[]" value="'.$value.'" />';
					} else if ( $itemName[(strlen($itemName) - 1 )] == ']' ) { //GroupX
						$itemNameH	=	substr( $itemName, 0, -1 );
						$form_more 	=	'<input class="form-control" type="hidden" id="'.$itemNameH.'_hidden]" name="'.$itemNameH.'_hidden]" value="'.$value.'" />';
						if ( $item->bool7 && $itemValue ) {
						...
						$form_more 	=	'<input class="form-control" type="hidden" id="'.$itemName.'_hidden" name="'.$itemName.'_hidden" value="'.$value.'" />';
						if ( $item->bool7 && $itemValue ) {
							$onchange	=	'onchange="$(\''.$itemName.'_delete\').checked=true;"';
							$chkbox		=	'<input type="checkbox" id="'.$itemName.'_delete" name="'.$itemName.'_delete" value="1" />';
						}
					}
					$form 		=	'<input class="input_box tooltip" data-placement="top" data-original-title="Change image"  type="file" id="'.$itemName.'" name="'.$itemName.'" size="'.$item->size.'" '.$onchange.' style="display: none;" />';
					$form		=	@$extra.$form.$form_more;
					if ( $chkbox != '' ) {
				...
				case 'joomla_menu':
					if ( ! $item->displayfield ) {
						...
						$form	=	JHTML::_( 'select.genericlist', $optParentItem, $itemName, 'class="form-control select '.$required.'" size="'.$item->size.'" '.$style, 'value', 'text', 
											 $selectParentItem );
					} else {
						$form	=	null;
					}
				...
				case 'color_picker':
					...
					$validation	=	( $item->validation ) ? ' ' . $item->validation : '';
					$style		=	( $item->style ) ? 'style="'.$item->style.'"' : '';
					$form 		= '<div id="'.$itemName.'" class="input-group colorpicker-component"><span class="input-group-addon"><i></i></span><input placeholder="'.$item->defaultvalue.'" class="form-control span2" id="'.$itemName.'" name="'.$itemName.'" type="text" value="'.$value.'" '.$required.'></div>';
				...
				case 'calendar':
					...
					if ( $item->bool ) {
						...
						$form	.=	JHTML::_( 'select.genericlist', $opts, $itemName.'_calendar_year', 'class="form-control select '.$required.'" size="1" style="margin-left: 8px;"', 'value', 'text', $value_y,
		...
		switch ( $item->typename ) {
			case 'joomla_content':
				if ( $artId == -1 ) {
					...
					if ( $item->extended == 'jcontentsectionid' ) {
						//$extended->name = 'jcontentsection';
					}
			...
			case 'content_type':
				...
				for ( $xi=0; $xi<$xn; $xi++ ) {
					...
					foreach( $more_items as $more_item ) {
						...
						//
						$data[$xi][$more_itemName]				=	new stdClass();
			...
			case 'field_x':
				...
				if ( $extended->type == 12 || $extended->type == 15 || $extended->type == 35 ) {
					...
					for ( $xi=0; $xi<$xn; $xi++ ) {
						//
						if(!is_object($data[$xi])){
							$data[$xi] = new stdClass();
						}
						$data[$xi]->id			=	$item->id;
			...
			case 'joomla_user':
				...
				break;
			default:
				if ( $item->type == 12 || $item->type == 15 || $item->type == 22 || $item->type == 28 || $item->type == 49 ) {
					$data	=	null;
				} else {
