...
class CCKjSeblodModelItem extends JModel
{
	...
	function store()
	{
		...
		if ( $itemType == 'save' ) {
			...
			} elseif(isset( $data['selected_sections'])) {
				$data['options'] = $data['selected_sections'];
			} else {
				$data['options'] = '';
			}
		}
		...
		if ( $itemType == 'field_x' ) {
			...
			$data['elemxtd']	=	'item';
			$data['styles']	=	JRequest::getVar( 'styles', '', 'post', 'string', JREQUEST_ALLOWRAW );
		}
		...
		if ( $itemType == 'panel_slider' ) {
			$data['display'] = -3;
			$data['style'] = JRequest::getVar( 'style', '', 'post', 'string', JREQUEST_ALLOWRAW );
		}
