			...
			switch ( $option ) {
				...
				case 'com_content':
					...
					if ( $joomfish ) {
						...
						$langs		=	CCK::DB_loadObjectList( 'SELECT * FROM #__jf_languages ORDER BY ordering ASC', 'sef' );
						...
						foreach ( $langs as $elem ) {
						  	$defaultImg = ( $elem->sef == $langDefault ) ? ' *' : '';
							if ( $elem->sef == $lang ) {
								$elems	.=	'<li><a class="active" href="index.php?option=com_content&lang='.$elem->sef.'">'
										.	$elem->lang_code.'</a></li>';
							} else {
								$elems	.=	'<li><a href="index.php?option=com_content&lang='.$elem->sef.'">'
										.	$elem->lang_code.'</a></li>';
							}
						}
