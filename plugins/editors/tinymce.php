	...
	function _displayButtons($name, $buttons)
	{
		...
		if(!empty($buttons))
		{
			...
			foreach ($results as $button)
			{
				...
				if ( $button->get('name') )
				{
					$modal		= ($button->get('modal')) ? 'class="btn btn-light-grey btn-file modal-button"' : null;
					$href		= ($button->get('link')) ? 'href="'.JURI::base().$button->get('link').'"' : null;
                    $onclick	= ($button->get('onclick')) ? 'onclick="'.$button->get('onclick').'"' : 'onclick="IeCursorFix(); return false;"';
					$return .= "<div class=\"button2-left\" style=\"padding: 5px 0;\"><div class=\"".$button->get('name')."\"><a ".$modal." title=\"".$button->get('text')."\" ".$href." ".$onclick." rel=\"".$button->get('options')."\">Insert Image</a></div></div>\n";
				}
			}
			$return .= "</div>\n";
		}
