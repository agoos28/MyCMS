				...
				if (  @name && @$name != 'readmorev'  )
				{
					...
					if ( $css != '' ) {
						if(!JApplication::isAdmin()){
							$modal	=	( $button->get( 'modal' ) ) ? 'class="btn btn-default '.$css.'" data-toggle="modal" data-target="iframemodal"' : null;
						}else{
						...
						if(!JApplication::isAdmin()){
							$modal	=	( $button->get( 'modal' ) ) ? 'class="btn btn-default" data-toggle="modal" data-target="iframemodal"' : null;
						}else{
