...
function plgSearchContent( $text, $phrase='', $ordering='', $areas=null )
{
	...
	$exclude 		= $pluginParams->def( 'exclude', 0 );
	...
	if(count($rows))
	{
		foreach($rows as $row)
		{
			$raw = getRawContent($row->text);
			$new_row = array();
			
			foreach($row AS $key => $article) {
				if(searchHelper::checkNoHTML($article, $searchText, array('text', 'title', 'metadesc', 'metakey'))) {
					$raw = getRawContent($article->text);
					$article->image = $raw->imagex[0];
					$article->price = $raw->pr_price;
					//$article->text = $raw->pr_text;
					$article->href = JRoute::_($article->href);
					$new_row[] = $article;				
				}
			}
