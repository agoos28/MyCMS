...
class UserModelReset extends JModel
{
	 ...
	 * @return	bool	True on success/false on failure
	 */
	function confirmReset($token, $email)
	{
		global $mainframe;
		...
		$db	= &JFactory::getDBO();
		$db->setQuery('SELECT id, activation FROM #__users WHERE block = 0 AND email = '.$db->Quote($email));
	...
	function completeReset($password1, $password2)
	{
		...
		$mainframe->setUserState($this->_namespace.'reqemail',	null);
	...
	function _sendConfirmationMail($email, $token)
	{
		...
		$uri		= &JFactory::getURI();
		$url		= JURI::base().'user/confirm-reset?email='.$email.'&token='.$token;
		$sitename	= $config->getValue('sitename');

		// Set the e-mail parameters
		$from		= $config->getValue('mailfrom');
		$fromname	= $config->getValue('sitename');
