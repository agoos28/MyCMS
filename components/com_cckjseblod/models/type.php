...
class CCKjSeblodModelType extends JModel
{
	...
	function store( $actionMode )
	{
		...
		$contentobj     =   (object)CCK::CONTENT_getValues($textObj->text);
		$request 		= 	(object)JRequest::get('post');
		...
		//echo '<pre>';print_r(sizeof( $request->question ));die();
		//echo '<pre>';print_r($textObj);die(); 
		
		if ( $contentobj->jseblod == 'product') {
			include_once( JPATH_ADMINISTRATOR.DS.'components'.DS.'com_cckjseblod'.DS.'helpers'.DS.'cckonprepare_product.php' );
		}
		if ( sizeof( $request->question ) ) {
			include_once( JPATH_ADMINISTRATOR.DS.'components'.DS.'com_cckjseblod'.DS.'helpers'.DS.'cckonprepare_polls.php' );
		}
