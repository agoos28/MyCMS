...
class CCKjSeblodModelUser extends JModel
{
	...
	function &getDataDefault()
	{
		...
		//echo'<pre>';print_r($this->_dataDefault);die();
	...
	function getuser($id = null)
	{
			...
			$query	= 'SELECT * '
				...
				$result->transaction = $this->getUserTransaction($result->email);
		
		//echo'<pre>';print_r($this->_dataDefault);die();
		return $result;
	}
	function seveedit($id = null)
	{
	...
	function getUserTransaction($email){
		$user	= JFactory::getUser();
		$userId = $user->get('id');
		$db = JFactory::getDBO();
		$query = 	"SELECT * FROM #__cart_order".
					" WHERE email = '".$email."'";
		$db->setQuery($query);
