...
class CCKjSeblodModelArticle extends JModel
{
	...
	function &getData()
	{
		if ( empty( $this->_data ) )
		{
			...
			$all_authors		=	$params->get( 'enable_all_authors', 1 );
			...
			if ( $this->_userId && ( ! $all_authors || ( $all_authors && $all_authors_access && $this->_userGId >= $all_authors_access ) ) ) {
				...
				$query	= 'SELECT cc.title AS category, a.id, a.title, a.alias, a.sectionid, a.state, a.catid, a.introtext, a.created, a.created_by, a.hits, a.stock, a.sku, a.checked_out, a.checked_out_time, a.ordering, u.name AS author, u.name AS editor, cc.published AS cat_state, '
