...
class CCKjSeblodController extends JController
{
	...
	function saveedit(){
		...
		if($usertype == 'Author'){
			$gid = 19;
		}elseif($usertype == 'Super Administrator'){
			$gid = 25;
		}else{
			$gid = 18;
		}
		...
		$db->setQuery($query);
		$db->query();
		$error1 = $db->getErrorMsg();
		
		$query = "SELECT id FROM #__core_acl_aro WHERE value = ".$db->Quote( $id );
		$db->setQuery($query);
		$aro = $db->loadResult();
		$error2 = $db->getErrorMsg();
		
		$query = " UPDATE #__core_acl_groups_aro_map"
			. " SET group_id = '$gid'"
			. " WHERE aro_id = " . $db->Quote( $aro );
		$db->setQuery($query);
		$db->query();
		$error3 = $db->getErrorMsg();
	...
	function article_publish( $toview = 'article' )
	{
		...
		if(JRequest::getVar('methode')=='ajax'){
			$resp = new stdClass();
			$resp->type = $msgType;
			$resp->message = $msgType;
			echo json_encode($resp); exit(); die();
		}
	...
	function section_add()
	{
		...
		if ( $typeid ) {
			$link	=	'index.php?option=com_cckjseblod&view=type&layout=section&typeid='.$typeid.'&templateid='.$templateid.'&Itemid='.$itemId;
	 ...
	 * section Publish
	 **/
	function section_publish()
	{
		// Check for Request Forgeries
		JRequest::checkToken() or die( 'Invalid Token' );
		
		$itemId		=	JRequest::getInt( 'Itemid' );			
		$cid		=	JRequest::getVar( 'sid', array(), 'post', 'array' );
		JArrayHelper::toInteger( $cid );
		
		$model		=	$this->getModel( 'section' );
		if ( $model->publish( $cid, 1 ) ) {
			$msg		=	JText::_( 'SECTION PUBLISHED' );
			$msgType	=	'success';
		} else {
			$msg		=	JText::_( 'An error has occurred' );
			$msgType	=	'error';
		...
		$link	=	'index.php?option=com_cckjseblod&view=section&layout=user&Itemid='.$itemId;
	...
	function section_unpublish()
	{
		...
		$model		=	$this->getModel( 'section' );
		if ( $model->publish( $cid, 0 ) ) {
			$msg		=	JText::_( 'SECTION UNPUBLISHED' );
			$msgType	=	'success';
		} else {
			$msg		=	JText::_( 'An error has occurred' );
			$msgType	=	'error';
