...
$jne = $model->getJne(JRequest::getVar('keyword'));
  ...
  <div class="col-md-12">
    <div class="panel panel-white">
      <div class="panel-heading border-light">
        <h4 class="panel-title text-bold">Local Shipment</h4>
          ...
          <li>
            <form class="pull-right" id="searchform" method="get" action="<?php echo JURI::current(); ?>">
              <div class="input-group">
                <input style="float: none;" name="keyword" class="form-control" value="<?php echo JRequest::getVar('keyword'); ?>" type="text" placeholder="Search Kota" />
      ...
      <div class="panel-body">
        <table class="table table-striped">
          <thead>
            <tr>
              ...
              <th align="left" class="sectiontableheader">Kecamatan</th>
              <th align="left" class="sectiontableheader">Kota</th>
              <th width="15%" align="center" class="sectiontableheader text-center">REG/Kg(IDR)</th>
              <th width="15%" align="center" class="sectiontableheader text-center">YES/Kg(IDR)</th>
              <th width="15%" align="center" class="sectiontableheader text-center">COD/Kg(IDR)</th>
              <th width="120" align="center" class="sectiontableheader text-center">Edit</th>
            </tr>
          </thead>
          <tbody>
            <?php foreach($jne->result as $item){ ?>
            <tr class="sectiontableentry<?php if($i % 2){ echo 2; }else{ echo 1; }; ?>" data-id="<?php echo $item->id; ?>">
              <td align="center"><?php echo $item->id; ?></td>
              <td align="left"><?php echo $item->kecamatan; ?></td>
              <td align="left"><?php echo $item->kota; ?></td>
              <td align="center" class="regular" data-name="reg"><?php echo $item->reg; ?></td>
              <td align="center" class="express" data-name="exp"><?php echo $item->yes; ?></td>
              <td align="center" class="cod" data-name="cod"><?php echo $item->cod; ?></td>
        ...
        <?php echo $jne->pageNav->getPagesLinks(); ?> </div>
    </div>
  </div>
</div>
...
function submitEdit(parent_el){
	...
	$.post('<?php echo JURI::current(); ?>', {'update':id, 'reg':regular, 'exp':express, 'cod': cod }, function(data){
		if(data == 'ok'){
			parent_el.find('.regular').html(regular)
			parent_el.find('.express').html(express)
			parent_el.find('.cod').html(cod)
			parent_el.find('a.edit').removeClass('hide')
			parent_el.find('a.save, a.cancel').addClass('hide')
		}else{
			alert('Failed')
