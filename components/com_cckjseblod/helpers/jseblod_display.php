...
class HelperjSeblod_Display
{
	...
	function quickModalWysiwyg( $text, $from, $into, $class, $required, $cid, $mode, $width = _MODAL_WIDTH, $height = _MODAL_HEIGHT )
	{
		...
		$modal	 	= 	"<div class=\"button2-left\"><div class=\"$class\"><a class=\"btn btn-xs btn-default\" title=\"\" href=\"$link\" rel=\"{handler: 'iframe', size: {x: $width, y: $height}}\">".$required.JText::_( $text )."</a></div></div>\n";
		$modal		=	$hidden . $modal;
	...
	function quickModalWysiwygJs( $text, $from, $into, $class, $required, $cid, $mode, $width = _MODAL_WIDTH, $height = _MODAL_HEIGHT )
	{
		...
		$link 		= 	JURI::base().'index.php?option=com_cckjseblod&amp;view=modal_wysiwyg'.$cid.'&amp;tmpl=component'.$from.$into.$mode;
		//$link		=	'javascript: SqueezeBox.fromElement(\''.$link.'\', {handler: \'iframe\', size: {x: '.$width.', y: '.$height.'}});';
		$modal	 	= 	'<div><a class="btn btn-primary modal-button" href="'.$link.'">Open Editor</a></div>';
		$modal		=	$hidden . $modal;
	...
	function quickModalImage( $text, $from, $into, $class, $width = _MODAL_WIDTH, $height = _MODAL_HEIGHT )
	{
		...
		$modal	 	= 	"<div class=\"button2-left\"><div class=\"$class\"><a class=\"btn btn-xs btn-default\" data-toggle=\"modal\" title=\"".JText::_( $text )."\" href=\"$link\" rel=\"{handler: 'iframe', size: {x: $width, y: $height}}\">".JText::_( $text )."</a></div></div>\n";
	...
	function quickModalUpload( $text, $from, $into, $class, $width = _MODAL_WIDTH, $height = _MODAL_HEIGHT )
	{
		...
		$modal	 	= 	"<div class=\"button2-left\"><div class=\"$class\"><a class=\"btn btn-xs btn-default\" title=\"".JText::_( $text )."\" data-toggle=\"modal\" href=\"$link\" rel=\"{handler: 'iframe', size: {x: $width, y: $height}}\">".JText::_( $text )."</a></div></div>\n";
