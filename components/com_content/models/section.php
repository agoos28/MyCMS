...
class ContentModelSection extends JModel
{
	...
	function _buildQuery($state = 1)
	{
		...
		$query = 'SELECT a.id, a.title, a.alias, a.stock, a.title_alias, a.introtext, a.fulltext, a.sectionid, a.state, a.catid, a.created, a.created_by, a.created_by_alias, a.modified, a.modified_by,' .
				' a.checked_out, a.checked_out_time, a.publish_up, a.publish_down, a.attribs, a.hits, a.images, a.urls, a.ordering, a.metakey, a.metadesc, a.access, bo.id AS booking_id, bo.deliver, bo.pickup, bo.book_status, ' .
				' CASE WHEN CHAR_LENGTH(a.alias) THEN CONCAT_WS(\':\', a.id, a.alias) ELSE a.id END as slug,'.
	...
	function _buildContentWhere($state = 1)
	{
		...
		if($gender){
			if($gender == 'laki-laki'){
				$where .= " AND a.gender = 1";
			}
			if($gender == 'perempuan'){
				$where .= " AND a.gender = 2";
			}
		}
		if($min){
		...
		if($params->get('orderby_sec') == 'special'){
	...
	function getBrands(){
		$query = 'SELECT brand' .
					' FROM #__content WHERE sectionid = '.$this->_id.
					' GROUP BY brand';
		$this->_db->setQuery($query);
    ...
    {	
		...
		if ( sizeof( $matches[1] ) ) {
			foreach ( $matches[1] as $key => $val ) {
				$regX =	'#\|\|'.$val.'\|\|(.*?)\|\|/'.$val.'\|\|#s';
				$gmatch = preg_match_all( $regX, $matches[2][$key], $xitem );
				if($gmatch > 1){
					$res[$val] = $xitem[1];
