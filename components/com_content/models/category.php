...
class ContentModelCategory extends JModel
{
	...
	function getSiblings()
	{
		...
		$this->_categories = $this->_siblings;
	...
	function _loadCategory()
	{
		...
		$this->_rawdesc = $this->getRawContent($this->_category->description);
		return true;
	}
	...
	function getEdit($contentType = null){
			...
			//return $contentType;
			$this->_type_id = 'index.php?option=com_cckjseblod&view=type&layout=form&typeid='.$contentTypeId.'&cckid='.$this->_id;
			return '<a class="btn btn-xs btn-red btn-edit" href="'.JRoute::_( 'index.php?option=com_cckjseblod&view=type&layout=form&typeid='.$contentTypeId.'&cckid='.$this->_id ).'" >edit</a>';
	}
	
	function getRawContent()
	{	
		$text = $this->_category->description;
		$res	=	array();
		$group	=	array();
		$regex	=	'#::(.*?)::(.*?)::/(.*?)::#s';
		$i = 0;
		...
		$res = (object)$res;
		$res->editlink = $this->getEdit($res->jseblod);
		$this->_raw = $res;
		return $res;
	}
	...
	function _buildQuery($state = 1)
	{
		...
		$query = 'SELECT cc.title AS category, a.id, a.title, a.alias, a.stock, a.title_alias, a.introtext, a.fulltext, a.sectionid, a.state, a.catid, a.created, a.created_by, a.created_by_alias, a.modified, a.modified_by,' .
			' a.checked_out, a.checked_out_time, a.publish_up, a.publish_down, a.attribs, a.hits, a.images, a.urls, a.ordering, a.metakey, a.metadesc, a.access, bo.id AS booking_id, bo.deliver, bo.pickup, bo.book_status, bo.order_id,' .
			...
			' FROM #__content AS a' .
			' LEFT JOIN #__booking AS bo ON bo.id = (
           SELECT MAX(id)
           FROM #__booking
           WHERE product_id = a.id AND ('.$this->_db->Quote($now).' BETWEEN deliver AND pickup) AND book_status = "processing"
        )' .
	...
	function _buildContentOrderBy($state = 1)
	{
		...
		if($menu->alias == 'best-seller'){
			$orderby .= $primary .' '. $secondary .' a.buycount DESC';
		}else{
			$orderby .= 'bo.pickup, ' . $primary .' '. $secondary .' a.created ASC';
		}
		return $orderby;
	}
	...
	function _buildContentWhere($state = 1)
	{
		...
		$available = JRequest::getVar('available');
    ...
    $where = ' WHERE 1';
		...
		//echo'<pre>';print_r($this->_category->sectionid == 12);die();
		if ($this->_category->sectionid == 12)
		{
			$where .= " OR a.group = ".(int) $this->_id;
		}
		
		if($brand){
			$where .= " AND a.brand = '".$brand."'";
		}
		if(($filters['gender_boy']) && ($filters['gender_girl'])){
				unset($filters['gender_boy']);
				unset($filters['gender_girl']);
			}
		if($filters){
			$filterField = array();
			$where .= " AND ( ";
			foreach($filters as $filter){
				$filterField[] = "(a.tag LIKE '%".$filter."%')";
			}
			$where .= implode(' OR ', $filterField);
			$where .= " ) ";

		}
		if($min){
			$where .= " AND a.price >= '".(int)$min."'";
		}
		if($max){
			$where .= " AND a.price <= '".(int)$max."'";
		}
		if($available){
			$where .= " AND bo.pickup IS NULL ";
		}
	...
	function getBrands(){
		$query = 'SELECT brand' .
					' FROM #__content WHERE catid = '.$this->_id.
					' GROUP BY brand';
		$this->_db->setQuery($query);
		$this->_brands = $this->_db->loadResultArray();
		return $this->_brands;
	}
	function getMin(){
		$query = 'SELECT MIN(price) AS minprice' .
					' FROM #__content WHERE catid = '.(int) $this->_id;
		$this->_db->setQuery($query);
		$this->_min = $this->_db->loadResult();
		return $this->_min;
	}
	function getMax(){
		$query = 'SELECT MAX(price) AS maxprice' .
					' FROM #__content WHERE catid = '.(int) $this->_id;
		$this->_db->setQuery($query);
