...
class JDocumentPDF extends JDocument
{
	...
	function render( $cache = false, $params = array())
	{
		...
		$pdf->Image($this->_mask, 70, 80, 70, '', '', '', '', false, 300, '', true);
		}
		$style = array('width' => 2, 'cap' => 'butt', 'join' => 'miter', 'dash' => 0, 'color' => array(123, 219, 196));

		$pdf->Line(0, 296, 215, 296, $style);
		if(JRequest::getvar('dl')){
			$data = $pdf->Output($this->getTitle().'.pdf', 'D');
		}else{
			$data = $pdf->Output($this->getTitle().'.pdf', 'S');
		}
